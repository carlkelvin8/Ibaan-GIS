...
const roleToCode = (input) => {
  ...
  if (/^\d+$/.test(s)) {
    ...
    return ROLE_CODE_TO_NAME[n] ? n : null;
  }
...
const coerceNullableIntForDb = (v) => {
  const n = safeInt(v);
...
function logSqlError(tag, err) {
  // Always print a helpful object; this avoids swallowing sqlMessage
...
export async function signup(req, res) {
  try {
    ...
    // ignore soft-deleted rows when checking dup
    ...
    logSqlError('signup', err);
    if (process.env.NODE_ENV !== 'production') {
      return res.status(500).json({ result: 0, error: err.sqlMessage || err.message });
    }
...
export async function updateUser(req, res) {
  try {
    ...
    const params = [
      first_name, last_name, email,
      roleCode, effectiveStatus, officeVal, muniVal,
    ];
...
export async function listFilteredUsers(req, res) {
  try {
    ...
    if (officeId !== undefined && officeId !== '') { where.push('u.office_id = ?'); params.push(coerceNullableIntForDb(officeId)); }
    if (municipalityId !== undefined && municipalityId !== '') { where.push('u.municipality_id = ?'); params.push(coerceNullableIntForDb(municipalityId)); }
    ...
    logSqlError('listFilteredUsers', err);
    return res.status(500).json({ error: err.message });
  }
}
...
export async function updateMyProfile(req, res) {
  try {
    ...
    const newVals = {
      ...
      office_id:  coerceNullableIntForDb(office_id ?? current.office_id),
      municipality_id: coerceNullableIntForDb(municipality_id ?? current.municipality_id),
    };
    ...
    logSqlError('updateMyProfile', err);
    if (process.env.NODE_ENV !== 'production') {
...
export async function changeMyPassword(req, res) {
  try {
    ...
    logSqlError('changeMyPassword', err);
    return res.status(500).json({ error: err.message });
  }
}
...
export async function forgotPassword(req, res) {
  ...
  try {
    ...
    logSqlError('forgotPassword', err);
...
export async function verifyResetToken(req, res) {
  ...
  try {
    ...
    logSqlError('verifyResetToken', err);
